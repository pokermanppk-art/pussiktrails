...
export default function AdminReservasRealizadas() {
  ...
  const carregarReservas = async () => {
    ...
    try {
      const { data: reservasData, error } = await supabase
        ...
        `)
        .eq('status', 'realizada')
        .order('data_trilha', { ascending: false })
      ...
      console.error('Erro ao carregar reservas realizadas:', err)
    } finally {
      setCarregando(false)
    }
  }
  ...
  const reservasFiltradas = reservas.filter((reserva) => {
    ...
    if (periodo !== 'todos') {
      const dataReserva = new Date(reserva.data_trilha)
      const hoje = new Date()
      ...
      >
        ...
        >
          <div style={{ backgroundColor: '#eff6ff', borderRadius: '20px', padding: '16px', textAlign: 'center', border: '1px solid #bfdbfe' }}>
            ...
            <div style={{ fontSize: '12px', color: '#6b7280' }}>Total Realizadas</div>
          </div>
          <div style={{ backgroundColor: '#f0fdf4', borderRadius: '20px', padding: '16px', textAlign: 'center', border: '1px solid #bbf7d0' }}>
            ...
            <div style={{ fontSize: '12px', color: '#6b7280' }}>Faturamento</div>
          </div>
          <div style={{ backgroundColor: '#fef3c7', borderRadius: '20px', padding: '16px', textAlign: 'center', border: '1px solid #fde68a' }}>
            <div style={{ fontSize: '28px', marginBottom: '8px' }}>👥</div>
            <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#f59e0b' }}>{estatisticas.totalPessoas}</div>
            <div style={{ fontSize: '12px', color: '#6b7280' }}>Aventureiros</div>
          </div>
          <div style={{ backgroundColor: '#f3f4f6', borderRadius: '20px', padding: '16px', textAlign: 'center', border: '1px solid #e5e7eb' }}>
            <div style={{ fontSize: '28px', marginBottom: '8px' }}>🥾</div>
            <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#6b7280' }}>{estatisticas.totalKm}</div>
        ...
        {carregando ? (
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '24px', padding: '60px', textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 'bold', color: '#374151', marginBottom: '6px' }}>Nenhuma reserva realizada</div>
            <div style={{ fontSize: '13px', color: '#6b7280' }}>
              {busca || periodo !== 'todos' ? 'Nenhuma reserva encontrada com estes filtros.' : 'As aventuras concluídas aparecerão aqui.'}
            </div>
          </div>
        ) : (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
                  ...
                  <div style={{ flex: 2 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap', marginBottom: '12px' }}>
                      ...
                      <div style={{ backgroundColor: '#3b82f6', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: '600' }}>
                        REALIZADA
                      </div>
                    </div>
                  ...
                  {/* LADO DIREITO - SELFIE BADGE */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ backgroundColor: '#dbeafe', padding: '8px 16px', borderRadius: '20px', fontSize: '12px', color: '#1e40af' }}>
                      🏁 Concluída
                    </div>
                  </div>
                </div>
