...
export default function AdminReservasPendentes() {
  ...
  const atualizarStatus = async (reservaId: string, novoStatus: string, reserva: Reserva) => {
    ...
    if (!error) {
      // Registrar atividade
      const adminNome = user.nome?.split(' ')[0] || user.email?.split('@')[0] || 'Admin'
      ...
      if (novoStatus === 'confirmada') {
        await registrarAtividade(
          ...
          adminNome,
          'confirmou_reserva',
          `${adminNome} confirmou a reserva de ${reserva.cliente_nome} para o roteiro "${reserva.roteiro_titulo}"`,
          reservaId
        )
        ...
        await registrarAtividade(
          ...
          adminNome,
          'cancelou_reserva',
          `${adminNome} cancelou a reserva de ${reserva.cliente_nome} para o roteiro "${reserva.roteiro_titulo}"`,
          reservaId
        )
      }
      
      setReservas((prev) => prev.filter((r) => r.id !== reservaId))
      alert(`Reserva ${novoStatus === 'confirmada' ? 'confirmada' : 'cancelada'} com sucesso!`)
    } else {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f3f4f6' }}>
      ...
      >
        ...
        >
          <div>
            <h1 style={{ margin: 0, fontSize: '24px', color: '#f59e0b', fontWeight: 'bold' }}>
              🟡 Reservas Pendentes
            </h1>
      ...
      >
        ...
        >
          <div style={{ backgroundColor: '#fffbeb', borderRadius: '20px', padding: '16px', textAlign: 'center', border: '1px solid #fde68a' }}>
            <div style={{ fontSize: '28px', marginBottom: '8px' }}>🟡</div>
            <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#f59e0b' }}>{estatisticas.total}</div>
            <div style={{ fontSize: '12px', color: '#6b7280' }}>Total Pendentes</div>
          </div>
          <div style={{ backgroundColor: '#f0fdf4', borderRadius: '20px', padding: '16px', textAlign: 'center', border: '1px solid #bbf7d0' }}>
            <div style={{ fontSize: '28px', marginBottom: '8px' }}>💰</div>
            <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#16a34a' }}>R$ {estatisticas.valorTotal.toFixed(2)}</div>
        ...
        {carregando ? (
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '24px', padding: '60px', textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 'bold', color: '#374151', marginBottom: '6px' }}>Nenhuma reserva pendente</div>
            <div style={{ fontSize: '13px', color: '#6b7280' }}>
              {busca ? 'Nenhuma reserva encontrada com esta busca.' : 'Todas as reservas foram confirmadas ou canceladas.'}
            </div>
          </div>
        ) : (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
                  ...
                  <div style={{ flex: 2 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap', marginBottom: '12px' }}>
                      ...
                      <div style={{ backgroundColor: '#f59e0b', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: '600' }}>
                        PENDENTE
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', fontSize: '13px', color: '#6b7280' }}>
                      ...
                      <span>📆 Solicitado: {new Date(reserva.created_at).toLocaleDateString('pt-BR')}</span>
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <button
                      onClick={() => atualizarStatus(reserva.id, 'confirmada', reserva)}
                      style={{
                    ...
                    <button
                      onClick={() => atualizarStatus(reserva.id, 'cancelada', reserva)}
                      style={{
                        backgroundColor: '#dc2626',
                        color: 'white',
                        border: 'none',
